...
export default function Prices() {
  ...
  const [encodedPath, setEncodedPath] = useState(''); // New state for encoded path
  const [selectedVehicleId, setSelectedVehicleId] = useState('placeholder_vehicle_id'); // Placeholder for vehicle ID

  useEffect(() => {
    const initMap = async () => {
      setIsLoading(true); // Set loading to true at the start of map initialization
      ...
      try {
        ...
        if (!mapDiv) {
          console.error('Map div element with ID "map" not found. Cannot initialize map.');
        ...
        directionsService.route(request, (result: google.maps.DirectionsResult | null, status: google.maps.DirectionsStatus) => {
          ...
          if (status === google.maps.DirectionsStatus.OK && result) {
            ...
            // Store the encoded polyline path
            ...
            toast.error('Could not retrieve route. Please try again.');
            setIsLoading(false);
          }
        });
      } catch (error) {
  ...
  const handleBookNow = () => {
    // Navigate to booking confirmation page with route details, vehicle_id, and path
    navigate('/booking', { 
      state: { 
        routeData, 
        routeDetails,
        vehicle_id: selectedVehicleId, // Pass the selected vehicle ID
        path: encodedPath // Pass the encoded path
      } 
    });
  };
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            ...
            <div className="lg:col-span-3 bg-white rounded-xl overflow-hidden shadow-lg relative"> {/* Added relative for absolute positioning of spinner */}
