...
export default function Booking() {
  ...
  const { routeData, routeDetails, vehicle_id, path } = (location.state as BookingState) || {};
  ...
  useEffect(() => {
    if (!routeData || !routeDetails || !vehicle_id || !path) {
      toast.error('Trip details not found. Please select a trip first.');
      navigate('/cabs'); // Redirect to the cabs selection page
    }
  }, [routeData, routeDetails, vehicle_id, path, navigate]);
  ...
  const handleConfirmBooking = async () => {
    ...
    if (routeData.tripType === 'Round Trip' && !dropDate) {
      toast.error('Please select a drop-off date for a round trip.');
      return;
    }

    setIsLoading(true);
    try {
      const token = localStorage.getItem('marcocabs_customer_token'); // Assuming token is stored here
      if (!token) {
      ...
      const payload = {
        ...
        dropoff_location: routeData.destination,
        pickup_date: pickupDate,
        drop_date: routeData.tripType === 'Round Trip' ? dropDate : pickupDate, // If one-way, drop_date can be same as pickup
        path: path,
        distance: parseFloat(routeDetails.distance.replace(' km', '')), // Convert "X km" to number
        // partner_id is optional, not included for now
      };
      ...
      if (response.data.success) {
        ...
        navigate('/previous-bookings'); // Redirect to previous bookings or a success page
      } else {
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-8 text-center">
            ...
            <p className="text-gray-600 mt-2">Review details and finalize your trip</p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-lg overflow-hidden mb-8">
            ...
            <div className="p-6 space-y-4">
              ...
              {routeData.tripType === 'Round Trip' && (
                <div>
                  <label htmlFor="dropDate" className="block text-sm font-medium text-gray-700 mb-2">Drop-off Date</label>
                  <input
                    type="datetime-local"
                    id="dropDate"
                    value={dropDate}
          ...
          <div className="flex flex-col sm:flex-row gap-4 mt-6">
            ...
            <button
              ...
              disabled={isLoading}
              className="flex-1 px-6 py-3 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-medium rounded-lg flex items-center justify-center transition-all shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
